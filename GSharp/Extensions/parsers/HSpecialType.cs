namespace GSharp.Extensions.parsers
{
    ...
    public static class HSpecialTypeExtensions
    {
        public static HSpecialType FromId(int id)
        {
            foreach (HSpecialType value in Enum.GetValues(typeof(HSpecialType)))
            {
                if ((int)value == id) return value;
            }
            return default(HSpecialType);
        }
    }
